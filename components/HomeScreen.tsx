...
export default function HomeScreen({ entries, sections }: { entries: Entry[]; sections: Section[] }) {
  ...
  const runSearch = useCallback(
    (q: string) => {
      ...
      setResults(r);
      setSearchOpen(r.length > 0 || q.trim().length > 2);
      setActiveIndex(-1);
    },
    [entries]
  );
  ...
  const handleKeyDown = (e: React.KeyboardEvent) => {
    ...
    if (e.key === 'ArrowDown') {
      ...
      if (target) {
        router.push(`/entry/${target.entry.id}`);
        setSearchOpen(false);
        setQuery('');
      }
    }
            ...
            <div className="absolute top-full left-0 right-0 z-50 border-2 border-t-0 border-[#0A0A0A] bg-white">
              {results.map(({ entry, matchType }, i) => {
                ...
                return (
                  ...
                  >
                    ...
                    <div>
                      {matchType === 'fuzzy' && (
                        <span className={`mr-1 text-sm font-normal ${i === activeIndex ? 'text-white/40' : 'text-[#0A0A0A]/30'}`}>~</span>
                      )}
                      <span
              ...
              {showGenerate && (
                <a
                  ...
                  className="flex items-center justify-between px-6 py-3 border-t border-dashed border-[#0A0A0A]/20 hover:bg-[#0A0A0A]/5 transition-colors"
                  onMouseDown={(e) => e.preventDefault()}
                  onClick={() => { setSearchOpen(false); setQuery(''); }}
                >
                  <span className="text-[10px] font-semibold tracking-[0.15em] uppercase text-[#0A0A0A]/50">
