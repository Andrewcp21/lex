...
export default function ConstellationGraph({ relatedIds, centerTerm, centerId, resolvedEntries }: Props) {
  const related = relatedIds
    .map((id) => resolvedEntries[id])
    .filter((r): r is ResolvedEntry => r !== undefined);
  ...
  const nodes = related.map((resolved, i) => {
    const baseAngleRad = ((i * (360 / n)) + startAngle(n)) * (Math.PI / 180);
    const h = termHash(resolved.entry.term);
    ...
    return { entry: resolved.entry, x, y, angle: angleRad, color: resolved.colorHex, lines: splitLines(resolved.entry.term) };
  });
  ...
  const l2Nodes = nodes.flatMap((l1Node) => {
    ...
    return candidates.slice(0, 1).map((id) => {
      ...
      return { entry: resolved.entry, x, y, parentX: l1Node.x, parentY: l1Node.y, color: resolved.colorHex, lines: splitLines(resolved.entry.term) };
  ...
  return (
    <div>
      ...
      <svg viewBox={viewBox} className="w-full h-auto">
        {l2Nodes.map((node, i) => (
          <line key={`l2-line-${i}`} x1={node.parentX} y1={node.parentY} x2={node.x} y2={node.y}
            stroke={node.color} strokeWidth={0.5} strokeOpacity={0.30} className="constellation-line" />
        ))}
        {nodes.map((node, i) => (
          <line key={`line-${i}`} x1={CX} y1={CY} x2={node.x} y2={node.y}
            stroke={node.color} strokeWidth={0.75} strokeOpacity={0.55} className="constellation-line" />
        ))}
        {l2Nodes.map((node, i) => {
          const rh = node.lines.length > 1 ? 20 : 13;
          return <rect key={`l2-bg-${i}`} x={node.x - L2_HALF_W} y={node.y - rh / 2} width={L2_HALF_W * 2} height={rh} fill="white" />;
        })}
        {nodes.map((node, i) => {
          const rh = node.lines.length > 1 ? 26 : 16;
          return <rect key={`bg-${i}`} x={node.x - NODE_HALF_W} y={node.y - rh / 2} width={NODE_HALF_W * 2} height={rh} fill="white" />;
        })}
        <rect x={CX - centerHalfW} y={CY - centerHalfH} width={centerHalfW * 2} height={centerHalfH * 2} fill="white" />
        {l2Nodes.map((node, i) => (
          <a key={`l2-node-${i}`} href={`/entry/${node.entry.id}`} style={{ cursor: 'pointer' }}>
            {node.lines.map((line, li) => {
              const yOffset = (li - (node.lines.length - 1) / 2) * 10;
              return (
                <text key={li} x={node.x} y={node.y + yOffset + 2.5} textAnchor="middle"
                  fontFamily="-apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif"
                  fontSize={8} fontWeight={700} letterSpacing="0.1em" fill={node.color} opacity={0.45}>
                  {line}
                </text>
              );
            })}
        ...
        {nodes.map((node, i) => (
          <a key={`node-${i}`} href={`/entry/${node.entry.id}`} style={{ cursor: 'pointer' }}>
            {node.lines.map((line, li) => {
              ...
              return (
                <text key={li} x={node.x} y={node.y + yOffset + 3} textAnchor="middle"
                  fontFamily="-apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif"
                  fontSize={10} fontWeight={700} letterSpacing="0.1em" fill={node.color}>
                  {line}
                </text>
              );
            })}
        ...
        {centerLines.map((line, li) => {
          ...
          return (
            <text key={li} x={CX} y={CY + yOffset + centerFontSize * 0.35} textAnchor="middle"
              fontFamily="-apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif"
              fontSize={centerFontSize} fontWeight={800} letterSpacing="0.08em" fill="#0A0A0A">
              {line}
            </text>
          );
        })}
